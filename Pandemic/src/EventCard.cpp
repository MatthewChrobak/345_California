...
void EventCard::eventAction(PlayerCard* card)
{
	...
	case EventCardType::Forecast:
	{
		...
		//Push the first 6 first cards of the infection card deck into the temporary deck
		for (int i = 0; i < 6; i++)
		{
	...
	//The next player to begin the playing the infector phase on their turn may skip the phase entirely
	case EventCardType::OneQuietNight:
	{
	...
	{
		/*
		 *User first chooses which player's pawn location to change
		 */

		int userChoiceInput;

		/*
		 *User sets the next location (integer)
		 */
		
		int locationInput;
		
		Game::getGameBoard()->getPlayer(userChoiceInput).pawn->cityIndex = locationInput;
