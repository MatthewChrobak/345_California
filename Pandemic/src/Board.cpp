...
void Board::generateGameContentAtStartOfGame()
{
    ...
    for (int i = 0; i < STARTING_INFECTION_CARD && i < infectionCityCards.size(); i++)
    {
        ...
        int val = infectionCityCards.at(0);

        Board::discardInfectionCard.push_back(val);
...
{
    int infectionRate = this->getInfectionRate();
    for (int i = 0; i < infectionRate; i++)
    {
        if (infectionCityCards.size() != 0)
        {
            ...
            int location = infectionCityCards.at(0);
            Board::discardInfectionCard.push_back(location);
            Board::infectionCityCards.erase(infectionCityCards.begin());
...
//better implementation of current player
...
void Board::set_players(const vector<Player *, std::allocator<Player *>> &_players) {
    Board::_players = _players;
}
...
void Board::setActionsAndRoles() {
    ...
    BuildResearchStation1* buildResearchStationMod = new  BuildResearchStation1();
    PlayerRoleCard* playerRoleCardOperation = new OperationsExpert(buildResearchStationMod);
    ...
    DiscoverCure1* discoverCureMod = new  DiscoverCure1();
    PlayerRoleCard* playerRoleCardScientist= new OperationsExpert(discoverCureMod);
    ...
    TreatDisease1* treatDiseaseMod = new  TreatDisease1();
    PlayerRoleCard* playerRoleCardMedic= new OperationsExpert(treatDiseaseMod);
    ...
    RoleCard* rc2 = new RoleCard("Scientist");

    _players[0]->setRoleCard(rc1);
    _players[1]->setRoleCard(rc2);
